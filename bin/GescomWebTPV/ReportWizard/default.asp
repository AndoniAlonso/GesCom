...
<html>
	...
	<body bgColor="#f5f5f5"  leftmargin=20>		
		...
		<%		
			...
			Response.Write ("<li><a href='default.asp?t=vTPVCierreCaja'>Cierre de Caja</a></li>")
			Response.Write ("<li><a href='default.asp?t=vTPVReportVentas'>Listado de ventas</a></li>")
			Response.Write ("</ul>")
...
'			Set objTable = Nothing	
			...
			%>
			<h2><%=strReportName%></h2>
			<form name=f1 action="<%=strUrl%>" method=post>
				<p>Click below to build you report or <a href="<%=strUrl%>">Click here!</a> to return </p>
				<input type=button onclick="OpenForm('<%=strTable%>','<%=strFields%>','<%=intPageSize%>')" value="Build Report">
				<input type=hidden name=fields>
				<input type=hidden name=pagesize>
				<input type=hidden name=criteria>
				<input type=hidden name=report_name>
				<input type=hidden name=save_report>
				<input type=hidden name=sfield>
				<input type=hidden name=stype>
				<input type=hidden name=sdesc>
				<input type=hidden name=sdesc2>
			...
			If strFields <> "" And strTable <> "" then	
				...
				Else
					...
					If stype<>"" and sfield<>"" Then
						...
						Response.Write("<tr style=""background-color:#ffffe0;font-family:comic sans ms;font-size:11px;""><td colspan="& (objRS.Fields.Count + 1) &"><p><font face=arial><b>Summary</b></font><br>")
					...
					If intCurrentPage > 1 Then
						Response.Write "<a href=""" & strUrl & "?" & strQueryTable & "&amp;" & strQuerySort & "&amp;cp="& (intCurrentPage-intPageSize) &"&amp;ps="& intPageSize &""">[&lt;&lt; Anterior]</a>&nbsp;"
					End If
					...
					If intCurrentPage < intPageCount Then
						Response.Write "<a href=""" & strUrl & "?" & strQueryTable & "&amp;" & strQuerySort & "&amp;cp="& (intCurrentPage+intPageSize) &"&amp;ps="& intPageSize &""">[Siguiente &gt;&gt;]</a>&nbsp;"
					End If
